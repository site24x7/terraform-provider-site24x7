...
resource "site24x7_port_monitor" "port_monitor_basic" {
    // (Required) Display name for the monitor
    display_name = "Example port Monitor"
...
// Site24x7 Port monitor API doc - https://www.site24x7.com/help/api/#port-(custom-protocol)
resource "site24x7_port_monitor" "port_monitor_us" {
    // (Required) Display name for the monitor
    display_name = "Example port Monitor"
    ...
    //(Optional)Should we prefer to use SSL or not
    use_ssl=true

    //(Optional)Choose the appropriate application type that runs on the configured port.
    application_type="FTP"

    //(Optional)Timeout for connecting to the host.Range 1 - 45.
    timeout=10 

    //(Optional) Use this to send plain text commands to the host.
    command=""
